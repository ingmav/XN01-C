...
export class TableroComponent implements OnInit {
  ...
  ngOnInit() {
    this.title.setTitle("TABLERO DE CONTROL GENERAL");
    ...
    var busquedaSubject = this.terminosBusqueda
        ...
        this.cargando = true;
	      return term  ? this.produccionService.buscar_detalle_tablero(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
	    }
  ...
  listar(paginate:number):void{
    ...
    this.cargando = true;
    this.produccionService.listarTablero(paginate,this.resultadosPorPagina).subscribe(
      resultado => {
  ...
  listarBusqueda(term:string ,pagina:number): void {
    ...
    this.cargando = true;
    this.produccionService.buscar_detalle_tablero(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
        resultado => {
          this.cargando = false;
  ...
  cambiaEstatus(tipo_produccion:number, estatus_produccion:number, id:number)
  {
    this.produccionService.actualiza_estatus(tipo_produccion, estatus_produccion, id).subscribe(
      resultado => {
        this.listar(1);
      },
      error => {
  ...
  {
    this.produccionService.actualiza_estatus(10, 1, id).subscribe(
      resultado => {
        this.listar(1);
      },
      error => {
